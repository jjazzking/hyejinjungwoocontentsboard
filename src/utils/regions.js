...
/** 시/군/구 토큰인지 ('고양시', '일산동구', '양평군') */
const isSigungu = (token) => /^[가-힣]{2,}(시|군|구)$/.test(token) && !SIDO_NAMES.has(token)

/**
 * 지번 동네 토큰을 표준형으로. 없으면 null.
 *   '성수동2가' → '성수동' / '역삼1동' → '역삼동' / '안덕면' → '안덕면'
 *   '신반포로11길'(도로명) · '종로1가'(로 이름) 는 동네가 아니므로 null
 */
 ...
 * → [{ key, label, fullLabel, sido, sigungu, dong, contents, places, lat, lng }]
