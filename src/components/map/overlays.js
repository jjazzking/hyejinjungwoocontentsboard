 ...
 * 380KB쯤 되므로 **경계를 처음 켤 때** 동적 import로 내려받는다
...
/**
 * 경계선 스타일: 밑의 지도를 가리지 않는 **굵은 실선** 하나.
 * 파선으로도 해 봤는데 지도 위에 무늬가 하나 더 얹힌 것처럼 보여서 실선으로 바꿨다.
 */
export const DISTRICT_STYLE = { color: '#334155', weight: 3, opacity: 0.55 }
...
export function loadDistrictBoundaries() {
  if (!cached) {
    cached = import('../../data/districtBoundaries.json')
      .then((module) => module.default.lines.map(decodeDistrictLine))
      .catch((error) => {
