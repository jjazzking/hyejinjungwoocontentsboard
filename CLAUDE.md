...
src/
  ...
  components/
    ...
    SettingsMenu.jsx          오른쪽 위 ⚙️ — 일괄 작업 3개를 접어 두는 드롭다운
                              (닫아도 안의 작업은 계속 돈다 — CSS로만 감춘다)
...
**일괄 작업은 오른쪽 위 톱니바퀴(`SettingsMenu`) 안에 모은다.** 예전엔 헤더에 그대로
깔려 있었는데 셋이 되고 나니 제목 아래가 버튼 줄로 시끄러워졌다. 가끔 정리할 때만 쓰는
것들이라 접어 둔다. **패널을 조건부 렌더링(`{open && …}`)으로 만들지 말 것** — 일괄
작업은 몇십 분을 돌 수 있는데 닫을 때 트리가 지워지면 Wake Lock이 풀리고 진행이 끊긴다.
`hidden` 클래스로 감추기만 하고, 도는 중에는 톱니바퀴에 점을 띄워 흔적을 남긴다
(`onBusyChange`). 메뉴 안의 버튼은 대상이 0이어도 숨기지 말고 비활성으로 둔다 —
열었는데 항목이 없으면 기능이 있는지조차 알 수 없다.
