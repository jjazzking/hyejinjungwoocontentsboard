...
src/
  ...
  components/
    ...
    SettingsMenu.jsx          ⚙️ 설정 서랍 — 가끔 쓰는 일괄 작업 버튼을 접어 둔다
...
**상단의 가끔 쓰는 작업은 `⚙️ 설정` 서랍(`SettingsMenu`) 안에 넣는다.** 링크로 카드 만들기·
일괄 재분석·시간대·원문 수집은 전부 여기다. 예전엔 헤더에 다 펼쳐 두었는데 보드를 열 때마다
카드보다 버튼이 먼저 보였다. 서랍은 겹쳐 뜨지 않고 아래로 밀며 열리고, **일괄 작업이 도는
동안에는 접히지 않는다** — 진행률이 안 보이면 끊긴 줄 알게 되기 때문이다
(각 버튼이 `onBusyChange`로 알려 준다).
